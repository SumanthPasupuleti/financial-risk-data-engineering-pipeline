...
# MAGIC %run "/Workspace/Users/sumanthpasupuleti9080@gmail.com/Consolidated_pip/cd Company/Utilities"
...
dbutils.widgets.text("catalog", "product", "Catalog")
dbutils.widgets.text("data_source", "orders", "Data Source")

catalog = dbutils.widgets.get("catalog")
data_source = dbutils.widgets.get("data_source")

base_path = f's3://pip-aws-spb-prj/{data_source}'
...
df = spark.read.options(header=True, inferSchema=True).csv(f"{landing_path}").filter("_metadata.file_name IS NOT NULL").withColumn("read_timestamp", F.current_timestamp()).select("*", "_metadata.file_name", "_metadata.file_size")
...
df_products = spark.table("product.silver.products")
...
# MAGIC %sql
# MAGIC DROP TABLE product.bronze.staging_orders;

# COMMAND ----------

# MAGIC %sql
# MAGIC DROP TABLE product.silver.staging_orders;