...
base_path = f's3://pip-aws-spb-prj/{data_source}/*.csv'
print(base_path)
...
# DBTITLE 1,Select only valid columns
df_silver = spark.sql(f"SELECT * FROM {catalog}.{silver_schema}.{data_source};")
...
delta_table = DeltaTable.forName(spark, "product.gold.dim_customers")
df_child_customers = spark.table("product.gold.sb_dim_customers").select(
...
# DBTITLE 1,Fix SyntaxError in delta_table merge
